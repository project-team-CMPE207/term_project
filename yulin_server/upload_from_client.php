...
if(!$file || !$from || !$title) {
  end_with_msg("required fields are missing for upload. Make sure you post these fields: $file, $from, $title");
}

// generate md5 id for uploaded file
$md5_id = md5_file($file["tmp_name"]);
if (!$md5_id) {
  end_with_msg("can't generate md5 id for uploaded file");
}
...
$success = move_uploaded_file($_FILES["file"]["tmp_name"], $upload_path) && db_insert_record($upload_path, "yulin's client", $md5_id, $title, $category, $desc);
if ($success) {
} else {
